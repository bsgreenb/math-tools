...
A **permutation** of a set $A$ is a bijection $A \rightarrow A$.
...
See [the symmetries of a square](https://proofwiki.org/wiki/Definition:Symmetry_Group_of_Square). Rotations are like if you stuck a pole through it like a pig on a spit.

## Permutations of a Finite Set

Let $a_1, a_2, \ldots, a_s$ be distinct elements of the set $\{1,2,\ldots, n\}$. By the **cycle** $(a_1a_2 \ldots a_s)$ we mean the permutation of $\{1,2,\ldots, n\}$ which carries $a_1$ to $a_2$, $a_2$ to $a_3$, $\ldots$, $a_{s-1}$ to $a_s$, and $a_s$ to $a_1$, while leaving all the remaining elements of $\{1,2,\ldots,n\}$ fixed. (Note the conection with modular addition: a cycle maps $a_i$ $a_{i+1 \mod s}$
...
If $(a_1a_2 \ldots a_s)$ is a cycle, the integer $s$ is called its **length**; thus, $(a_1a_2 \ldots a_s)$ is a cycle of length $s$.
...
**Cycle Decomposition Theorem**: $\alpha^k$ is the product of $d=\operatorname{gcd}(s, k)$ disjoint cycles of length $t=\frac{s}{d}$. Specifically:

$$
\alpha^k=\prod_{m=1}^d(m, m+k, m+2 k, \ldots, m+(t-1) k)
$$

From this we can see that when $s$ and $k$ are coprime ($\operatorname{gcd}(s, k) = 1$), $\alpha^k$ will be a single cycle. Furthmore, when $s$ is prime, the only possible $\operatorname{gcd}(s, k)$ is either 1 (another single cycle) or $s$, in which case we have cycles of length 1 -- resulting in the identity permutation.
...
If $\alpha$ and $\beta$ are disjoint cycles of lengths $r$ and $s$, respectively, the order of $\alpha\beta$ is $\text{lcm}({\alpha\beta})$.

## Isomorphism

Isomorphic things have the same structure.

Let $G_1$ and $G_2$ be groups. A bijective function $f: G_1 \rightarrow G_2$ with the property that for any two elements $a$ and $b$ in $G_1$,
...
### Some General Properties of Isomorphism
...
If $\operatorname{ord}(a)=k m$, then $\operatorname{ord}\left(a^k\right)=m$.

If $\operatorname{ord}(a)=n$ where $n$ is odd, then $\operatorname{ord}\left(a^2\right)=n$.

### Relationship between ord(ab), ord(a), and ord(b)

Let $a$ and $b$ be elements of a group $G$. Let $\operatorname{ord}(a)=m$ and $\operatorname{ord}(b)=n$; let $\operatorname{lcm}(m$, $n$ ) denote the least common multiple of $m$ and $n$.

If $a$ and $b$ commute, then $\operatorname{ord}(a b)$ is a divisor of $\operatorname{lcm}(m, n)$.
...
Let $a$ and $b$ commute. If $m$ and $n$ are relatively prime, then $\operatorname{ord}(a b)=m n$.

Let $a$ and $b$ commute. There is an element $c$ in $G$ whose order is $\operatorname{lcm}(m, n)$.

Thus, there is no simple relationship between $\operatorname{ord}(a b)$, $\operatorname{ord}(a)$, and $\operatorname{ord}(b)$ if $a$ and $b$ fail to commute.

### Relationship between ord(a) and ord(a^k)

Let $a$ be an element of order $n$ in a group $G$.

If and only if $m$ and $n$ are relatively prime, $a^m$ has order $n$.

The order of $a^m$ is $\frac{\text{lcm}(m,n)}{m}$ , which is more typically stated as:
...
Note that $l$ can be 1 when every prime factor of the root index $k$ is also a factor of the original order $n$, resulting in the maximum possible order for the root $b$ ($nk$).
...
**Theorem: Isomorphism of cyclic groups**
...
(ii) Every cyclic group of order infinity is isomorphic to $\mathbf{2}$, and therefore any two cyclic groups of order infinity are isomorphic to each other.

**Theorem:** Every subgroup of a cyclic group is cyclic.

The order of a cyclic group is the same as the order of its generator.

You cannot have an uncountably infinite cyclic group (e.g. $\mathbb{R}^*$), only countably infinite.

### Elementary Properties of Cyclic Groups

If $G$ is a group of order $n$, $G$ is cyclic if and only if $G$ has an element of order $n$.
...
For any positive integer $n$, let $\phi(n)$ denote the number of positive integers less than $n$ and relatively prime to $n$ (**Euler's totient function**). For example, $1, 2, 4, 5, 7, 8$ are relatively prime to $9$, so $\phi(9) = 6$.

Let $a$ have order $n$. Then:

- $a^r$ is a generator of $\langle a\rangle$ iff $r$ and $n$ are relatively prime.
- $\langle a\rangle$ has $\phi(n)$ different generators.
- For any factor $m$ of $n$, let $C_m = \{x \in \langle a\rangle : x^m = e\}$. Then $C_m$ is a subgroup of $\langle a\rangle$ (with exactly $m$ elements).
- An element $x$ in $\langle a\rangle$ has order $m$ iff $x$ is a generator of $C_m$. Consequently, there are $\phi(m)$ elements of order $m$ in $\langle a\rangle$.
- If $c$ is any generator of $\langle a\rangle$, then $\{c^r : r \text{ is relatively prime to } n\}$ is the set of all generators of $\langle a\rangle$.

### Elementary Properties of Cyclic Subgroups of Groups

Let $G$ be a group and let $a, b \in G$.

- If $a$ is a power of $b$, say $a = b^k$, then $\langle a\rangle \subseteq \langle b\rangle$, with equality iff $b$ is also a power of $a$.
- Suppose $a \in \langle b\rangle$. Then $\langle a\rangle = \langle b\rangle$ iff $a$ and $b$ have the same order.
- Let $\operatorname{ord}(a) = n$. If one of $a, b$ is a $k$th power of the other (i.e. $b = a^k$ or $a = b^k$), then $\langle a\rangle = \langle b\rangle$ iff $k$ and $n$ are relatively prime.
- Any cyclic group of order $mn$ has a unique subgroup of order $n$.

### Direct Products of Cyclic Groups

Let $G$ and $H$ be groups, with $a \in G$ and $b \in H$.

- If $(a, b)$ is a generator of $G \times H$, then $a$ is a generator of $G$ and $b$ is a generator of $H$. Consequently, if $G \times H$ is cyclic, then both $G$ and $H$ are cyclic. The converse is false: $G$ and $H$ both cyclic does *not* imply $G \times H$ is cyclic.
- Let $\operatorname{ord}(a) = m$ and $\operatorname{ord}(b) = n$. The order of $(a, b)$ in $G \times H$ is $\operatorname{lcm}(m, n)$.
