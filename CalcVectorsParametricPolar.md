...
Time ($t$) tends to be the parameter used, but it could be something else.
...
Let's start with the magic differential approach taken on the [Khan Vid](https://www.khanacademy.org/math/ap-calculus-bc/bc-advanced-functions-new/bc-9-3/v/parametric-curve-arc-length?modal=1):

![image.png](images/parametric-arc-length.png)
...
But the more rigorous proof is on [Paul's Math Notes](https://tutorial.math.lamar.edu/classes/calcii/ParaArcLength.aspx).  It does some funky stuff with absolute value I'm not completely groking.

# Polar Curves/Equations

Good [Overview on Brilliant](https://brilliant.org/wiki/polar-curves/). 
...
One advantage of using polar equations is that certain relations that are not functions in Cartesian form can be expressed as functions in polar form.

## Derivatives of Polar Curves

Derivatives of Polar Curves
To find $\frac{d y}{d x}$ given that $x=r \cos \theta$ and $y=r \sin \theta$, we can find $\frac{d y}{d \theta}$ and $\frac{d x}{d \theta}$. Upon dividing the former by the latter, we get $\frac{d y}{d x}$.
...
- When $\frac{d y}{d \theta}=0$, we have horizontal tangent lines.
- When $\frac{d x}{d \theta}=0$, we have vertical tangent lines.

## Areas Bound by Polar Curves

Whereas with a cartestian graph we'd use rectangles to get an integral, with a polar graph it's more taking a bunch of rotating pie pieces.  Infinitely thin pie slices.  Area of these sectors given by:

$$\frac{\theta}{2\pi}\pi r^2 = \frac{\theta}{2}r^2 = \frac{1}{2}r^2\theta$$

$\theta$ -> $d\theta$ as slice gets infinitely small..  So we can calculate the slices over a given angular interval as:

$$ \int_\alpha^\beta \frac{1}{2}r^2~d\theta $$

![image.png](CalcVectorsParametricPolar_files/928f7302-b688-4ca8-ae19-0522aa577d21.png)

So if you're calculating the area between polar curves, the key thing will be to identity where those curves intersect, and use that to determine your integrals.  

Be on the lookout for symmetries when you're looking at areas between curves.
